...
This is an example of simulating using `simEventData`. The `N` argument lets the user specify number of individuals in the simulation. We can simulate observational health care data for 100 individuals with the function call
...
`simEventData2` is a function for more generel event history simulations. The structure and the arguments of the function resemble `simEventData`. The number of events simulated is determined by the length of the `eta` vector, `nu` vector or the number of columns in the `beta` matrix. We specify the beta matrix as
```{r}
