...
You can also specify non-linear effects, e.g., effect of \code{N2} on \code{N1} to increase by 2 if \code{N2 > 1}:
...
We can sanity check that the argument performs as it should
```{r}
data_int <- IntFormatData(data, N_cols = 8:12)
data_int[, NL1 := as.numeric(N2> 1)] 

coxph(Surv(tstart, tstop, Delta == 1) ~ L0 + A0 + Z1 + Z2 + N2 + N3 + N4 + NL1, data = data_int)$coefficients |> knitr::kable()
```

Additionally the argument can be used to specify interaction effects, e.g., if we want an interaction effect of 1 between L0 and N3 on N1 we can specify the argument:
...
A further example of the arguments use, is that we can use it to specify effects that depend on time. E.g. if we wish for an additional effect on N2 of 0.4 if T1 > 1, then we can simulate as follows
...
coxph(Surv(tstart, tstop, Delta == 2) ~ L0 + A0 + Z1 + Z2 + N2 + N3 + N4 + Z, data = data_int[N2 < 2])$coefficients |> knitr::kable()
```
...
By default the function returns the proportion of individuals who experience death and the proportion of individuals who experience Drop In by a specified time \eqn{\tau} in group \code{A0 = a0}. The user can specify what gets returned by the function call with use of the arguments `years_lost` and `return_data`. If the  former is set to TRUE the function returns number of years lost before \eqn{\tau} of death and Drop In. If the latter is set to TRUE the function call returns the simulated data. 

To compare a non intervened scenario with a non intervened scenario, the function `intEffectAlphaTreat` can be used. This function simulates data from the Drop In setting in two scenarios. Under the intervention on the shape parameter \eqn{\eta} of the Drop In process, which is multiplied by \code{alpha}, and a baseline (non-intervened) scenario. It computes the proportion of individuals who experience death or Drop In by a specified time \eqn{\tau} in the group \code{A0 = a0}, optionally returning years lost. The function can also plot a sample of the event data for each scenario for comparison.
